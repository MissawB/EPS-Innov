...
def check_and_request_permissions():
    sdk_version = int(autoclass('android.os.Build$VERSION').SDK_INT)
    if sdk_version >= 23:  # Runtime permissions required from Android 6.0+
        request_permissions([
...
class SelectionScreen(Screen):
    def __init__(self, **kwargs):
        ...
        btn_basket = Button(text="Basket",font_size=sp(Window.width * 0.03))
        btn_basket.bind(on_release=self.go_to_basket)
        btn_layout.add_widget(btn_basket)

        btn_futsal = Button(text="Futsal",font_size=sp(Window.width * 0.03))
...
# Écran de futsal
class FutsalScreen(Screen):
    def __init__(self, **kwargs):
        super(FutsalScreen, self).__init__(**kwargs)
    ...
    def update_observables_layout(self, player):
        ...
        default_observables = ['But', 'Passe décisive', 'Duel Gagné']
...
class FeaturesScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.timer_label = Label(text="Chrono: 00:00", font_size=24, size_hint=(0.4, 1))
        top_layout.add_widget(self.timer_label)

        # Bouton de mode
        self.mode_button = Button(text="Mode Timer", size_hint_x=0.2, height=50)
        self.mode_button.bind(on_release=self.switch_mode)
        top_layout.add_widget(self.mode_button)
        ...
        self.start_timer_button = Button(text="Démarrer le Chrono", size_hint=(1, None), height=50)
        self.start_timer_button.bind(on_release=self.start_timer)
        main_scroll_content.add_widget(self.start_timer_button)
    ...
    def update_timer(self, dt):
        ...
        minutes, seconds = divmod(abs(self.start_time), 60)
        hours, minutes = divmod(minutes, 60)
        self.timer_label.text = f"{'Chrono' if self.is_timer_mode else 'Timer'}: {hours:02}:{minutes:02}:{seconds:02}"
    ...
    def stop_timer(self):
        if self.timer_event:
            Clock.unschedule(self.timer_event)
            self.timer_event = None
            self.start_timer_button.text = "Démarrer le Chrono" if self.is_timer_mode else "Démarrer le Timer"
    ...
    def show_chrono_popup(self):
            ...
            popup = Popup(title="Réglage de la durée du Timer", content=layout, size_hint=(0.8, 0.8))
